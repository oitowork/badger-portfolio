...
export const GET_BLOCKS = (timestamps: { map: (arg0: (timestamp: any) => string) => string; }) => {
  let queryString = 'query blocks {';
  queryString += timestamps.map((timestamp: number) => {
...
export const POSITIONS_BY_BLOCK = (account: any, blocks: { map: (arg0: (block: any) => string) => string; }) => {
  let queryString = 'query blocks {';
  queryString += blocks.map(
    (block: { timestamp: any; number: any; }) => `
...
export const PRICES_BY_BLOCK = (tokenAddress: any, blocks: { map: (arg0: { (block: any): string; (block: any): string; }) => string; }) => {
  let queryString = 'query blocks {';
  queryString += blocks.map(
    (block: { timestamp: any; number: any; }) => `
...
export const HOURLY_PAIR_RATES = (pairAddress: any, blocks: { map: (arg0: (block: any) => string) => string; }) => {
  let queryString = 'query blocks {';
  queryString += blocks.map(
    (block: { timestamp: any; number: any; }) => `
...
export const SHARE_VALUE = (pairAddress: any, blocks: { map: (arg0: { (block: any): string; (block: any): string; }) => string; }) => {
  let queryString = 'query blocks {';
  queryString += blocks.map(
    (block: { timestamp: any; number: any; }) => `
      t${block.timestamp}:pair(id:"${pairAddress}", block: { number: ${block.number} }) { 
...
export const USER = (block: any, account: any) => {
  const queryString = `
    query users {
...
export const PAIR_DAY_DATA_BULK = (pairs: any[], startTimestamp: any) => {
  let pairsString = `[`;
  pairs.map((pair: any) => {
...
export const GLOBAL_DATA = (block: any) => {
...
export const PAIR_DATA = (pairAddress: any, block: any) => {
  const queryString = `
    ${PairFields}
    query pairs {
...
export const PAIRS_HISTORICAL_BULK = (block: any, pairs: any[]) => {
  let pairsString = `[`;
  pairs.map((pair: any) => {
    return (pairsString += `"${pair}"`);
  });
...
export const TOKENS_DYNAMIC = (block: any) => {
  const queryString = `
    ${TokenFields}
    query tokens {
